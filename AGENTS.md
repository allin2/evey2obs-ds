...
本项目的开发组合是 <strong>Codex + DeepSeek V4 Pro</strong>：Codex 提供代码编辑、工具调用、任务编排与项目记忆，实际底层推理模型为 `deepseek-v4-pro`。
这不是“Codex 主模型 + DeepSeek 第二模型”的双模型编排，而是 Codex 运行在 DeepSeek V4 Pro 模型上。
项目级 `.Codex/settings.json` 固定非敏感的网关与模型名，认证令牌仅保存在用户级 Codex 配置中。
...
- DeepSeek V4 Pro 或 Anthropic 兼容端点不支持某项 Codex 能力时，明确说明限制，不伪造工具结果。
